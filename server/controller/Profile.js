...
exports.getEnrolledCourses = async (req, res) => {
  try {
    ...
    // Fetch user details and populate courses, courseContent, and SubSection
    let userDetails = await User.findOne({ _id: userId })
      .populate({
        path: "courses",
        populate: {
          path: "courseContent",
          model: "Section", // Reference the Section model
          populate: {
            path: "SubSection",
            model: "Subsection", // Reference the SubSection model
          },
        },
      })
      .exec();
    ...
    for (let i = 0; i < userDetails.courses.length; i++) {
      ...
      if (userDetails.courses[i].courseContent) {
        for (let j = 0; j < userDetails.courses[i].courseContent.length; j++) {
          const courseContent = userDetails.courses[i].courseContent[j];

          // Safely check if subSection exists
          if (courseContent.SubSection) {
